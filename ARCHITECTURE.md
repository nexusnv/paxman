...
Subsystem boundaries are enforced by lint rules and review. See [PACKAGE_STRUCTURE.md §8 System Boundary Rules](./PACKAGE_STRUCTURE.md) for the formal rules and how to enforce them.

### 3.4 Field-centric, not document-centric

Paxman plans resolution **independently for each required field**. There is no "parse the document" mega-step. This enables cost optimization, targeted inference, and selective capability invocation. See [ADR-0001](./docs/adr/0001-field-centric-planning.md).

### 3.5 Capability confidence rule

Capabilities **never** assign confidence. They return candidates + evidence + diagnostics. Confidence is assigned **only** by the Reconciler; the Planner emits a `target_confidence` (read from the field's `confidence_threshold`) but never scores a candidate. This prevents confidence inflation. See [ADR-0005](./docs/adr/0005-confidence-ownership.md).

### 3.6 The pipeline is synthesized, not fixed

Paxman does not have a single canonical pipeline. It synthesizes a plan per (contract, input) pair. Different inputs may use different capabilities for the same field. See [ADR-0002](./docs/adr/0002-rule-based-planner-v1.md).
...
This ordering is a **default**; the planner can be overridden per contract via `ResolutionPolicy` on a field. See [ADR-0002](./docs/adr/0002-rule-based-planner-v1.md).

**FieldPlan shape** (informal; see [PACKAGE_STRUCTURE.md §planner](./PACKAGE_STRUCTURE.md) for module-level details):
...
**Capability contract** (informal; see [PACKAGE_STRUCTURE.md §capabilities](./PACKAGE_STRUCTURE.md)):
...
**Critical rule:** capabilities never assign confidence. See [ADR-0005](./docs/adr/0005-confidence-ownership.md).
...
For module-level details, see [PACKAGE_STRUCTURE.md](./PACKAGE_STRUCTURE.md).
...
- Confidence is assigned **only** in the Reconciler. The Planner may emit a `target_confidence` (the field's `confidence_threshold`) but never assigns confidence to a candidate. See [ADR-0005](./docs/adr/0005-confidence-ownership.md).
...
- All 9 success metrics in [PRD §9](./PRD.md) are met or explicitly waived.
- All 8 V1 acceptance criteria in [PRD §10](./PRD.md) are met.
...
See [TESTING_STRATEGY.md](./TESTING_STRATEGY.md) for the full strategy. This section lists the architectural seams.
...
This decision is to keep V1 deterministic and replayable end-to-end without a complex scheduler. See [ADR-0006](./docs/adr/0006-sequential-execution-v1.md).
...
See [SECURITY.md](./SECURITY.md) for the full threat model. This section is the architectural summary.
...
These are tracked in [PRD §13](./PRD.md) as open questions for the V1 cycle. Architectural-level questions:
...
3. **Q-A3** Should `MONEY` arithmetic be a capability or a Reconciler primitive? *(Open — likely a Reconciler primitive; see [ADR-0004](./docs/adr/0004-money-first-class-type.md).)*
...
- [PACKAGE_STRUCTURE.md](./PACKAGE_STRUCTURE.md) — Module layout, dependency DAG, public/private split.
