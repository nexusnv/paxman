...
sequenceDiagram
    ...
    CAP->>PA: parse_amount amount currency parts canonical
    PA->>PA: apply separator convention comma vs dot
    PA->>PA: _validate_thousands rejects ambiguous grouping
    PA->>PA: Decimal exact preserve decimals normalize sci notation
    ...
    CAP->>CAP: compose canonical currency colon sign parsed
    CAP->>RULES: currency_from_contract symbol code validated preserved_sign parsed_decimal preserved_decimals canonical_form
    RULES-->>CAP: Evidence rule authority from manifest
    CAP-->>API: CapabilityResult CANONICALIZED value evidence
    ...
    ART->>ART: VersionStamp paxman contract capabilities
    ...
    API->>ART: re-canonicalize to identical artifact
```
