...
sequenceDiagram
    ...
    alt parse_contract raises ContractError
        ...
        alt no claimants
            ...
            Note over CAP,RULES: Stage 3+4 - execute + canonicalize
            alt not a CanonicalMoneyContract
                CAP-->>RULES: _evidence("not_a_money_contract", engine)
                RULES->>RULES: lookup manifest authority (None for routing guard)
                RULES-->>CAP: Evidence(rule, authority=None)
                CAP-->>API: CapabilityResult(status=INVALID)
            else value not None and not a str
                CAP-->>RULES: _evidence("not_a_string_value", engine)
                RULES-->>CAP: Evidence(authority=None)
                CAP-->>API: CapabilityResult(status=INVALID)
            else value is None or whitespace only
                ...
                CAP->>CAP: value.strip() if changed
            ...
            CLS-->>API: final Status (CANONICALIZED here)
            ...
            ART->>ART: candidates only when AMBIGUOUS else None
    ...
    alt parse_contract raises ContractError
        ...
        REPLAY->>REPLAY: verify version_stamp.paxman_version == current
        REPLAY->>REPLAY: verify version_stamp.contract_version == parsed.version_field
        REPLAY->>REG: capabilities_hash()
        REG-->>REPLAY: sha256 of sorted capability names
        REPLAY->>REPLAY: verify version_stamp.capabilities_hash == registry hash
        REPLAY->>REPLAY: verify artifact.replay_hash == sha256(canonical_bytes())
        ...
        ENG-->>REPLAY: ok or raise UnknownAuthorityEdition
...
- **MoneyCapability (money/canonicalizer.py)**: pure `(value, contract, engine) -> CapabilityResult` transform. Guards order: contract type -> value type -> missing -> strip -> recognize -> parse -> compose.
...
- **classify (core/classification.py)**: maps the `CapabilityResult` + `ValidationResult` to the final `Status`.
...
- **replay (core/replay.py)**: re-parses the contract, verifies the `VersionStamp`, verifies `capabilities_hash` against the live registry, verifies `replay_hash == sha256(canonical_bytes())`, reconstructs the engine via `Engine.from_artifact`, then returns the artifact **unchanged**. It never re-runs `canonicalize`.
...
- Replay does NOT re-canonicalize — it verifies and rehydrates. Any version/hash mismatch raises `VersionMismatchError` or `CanonicalizationError`.
