...
| [0012](./0012-v1-capabilities-self-register-on-import.md) | V1 Capabilities Self-Register on Import | Accepted | 2026-07-01 | All **10** V1 capabilities self-register on import (5 V1.0.0 originals + 3 V1.1.0 format-aware extractors per ADR-0015 + 2 V1.1.0 post-extraction cleanup transforms per ADR-0014), mirroring the 4 contract adapters. `paxman.register_capability()` stays public for third-party extensions. |
...
| [0015](./0015-format-aware-executor-auto-dispatch.md) | V1.1.0 Format-Aware Executor Auto-Dispatch | Accepted | 2026-07-01 | `FormatHint` on the contract (one of `CSV` / `JSON` / `XML`) drives the planner to prepend the matching format-aware extractor to the local-deterministic chain, closing the documented gap where a CSV / JSONPath / XPath field was dispatched to the V1.0.0 `text_extraction` + `regex_extraction` chain and returned `UNRESOLVED`. The dispatch is opt-in (default `format_hints=()`) and member-agnostic. |
