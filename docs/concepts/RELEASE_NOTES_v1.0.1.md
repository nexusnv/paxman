...
| Golden artifacts | **Unchanged under 1.0.1 itself** — none of the 8 `tests/fixtures/artifacts/*.json` files were regenerated by the three v1.0.1 bug fixes. (Subsequent v1.1.0 work on this branch — the capability bootstrap fix for #79 / #80 — does regenerate the goldens; see `CHANGELOG.md` → `[Unreleased]`.) |
...
**Issue:** [#56](https://github.com/nexusnv/paxman/issues/56)
...
**Issue:** [#57](https://github.com/nexusnv/paxman/issues/57)
...
**Issue:** [#54](https://github.com/nexusnv/paxman/issues/54)
...
**Issue:** [#55](https://github.com/nexusnv/paxman/issues/55)
...
- All 8 `tests/fixtures/artifacts/*.json` goldens are unchanged
  by the v1.0.1 patch itself. (They are regenerated by the
  v1.1.0 capability bootstrap fix in #79 / #80 — a separate
  change tracked in `CHANGELOG.md` → `[Unreleased]`.)
