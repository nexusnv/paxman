...
one call. ``make_evidence_for`` extends it for the *trial* engine-aware
capabilities (country, money): when an ``engine`` binds a non-default edition of
a named authority, the registry-citing rules re-resolve their authority from the
engine so the recorded edition matches the binding (Concern 3). The authority is
named by a plain string param — not hardcoded — so a future 3rd registry-backed
...
def make_evidence_for(
    ...
    When ``engine`` is not ``None`` and ``rule`` is in ``registry_rules``, the
    authority is re-resolved from ``engine.authority(authority_name)`` so the
    recorded edition reflects the pinned edition. ``authority_name`` is a plain
    string (e.g. ``"ISO 3166-1"``) — not hardcoded — to stay flexible as
    engine-awareness spreads to more domains.
    ...
    def _evidence(rule: str, detail: str = "", engine: Engine | None = None) -> Evidence:
        ...
        if engine is not None and rule in rules:
