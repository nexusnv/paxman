...
def test_recognize_grammars_no_match_returns_empty():
    assert recognize_grammars(_fake_grammars(), "not-a-uuid", _FakeContract(), _FakeContract) == []
...
def test_recognize_grammars_strip_charset_is_narrow():
    ...
    reps = recognize_grammars((g,), "  abc  ", _FakeContract(), _FakeContract, strip=" \t\r\n\f\v")
    assert len(reps) == 1
    assert reps[0].raw == "abc"
    # A non-breaking space is NOT in the narrow charset → preserved → no match.
    assert (
        recognize_grammars((g,), "abc\u00a0", _FakeContract(), _FakeContract, strip=" \t\r\n\f\v")
        == []
    )
